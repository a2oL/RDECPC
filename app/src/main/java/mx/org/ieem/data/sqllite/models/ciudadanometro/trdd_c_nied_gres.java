...
public class trdd_c_nied_gres implements Parcelable
{
    private int id_nivel_educativo;
    private int id_grado_escolar;

    public trdd_c_nied_gres(int id_nivel_educativo, int id_grado_escolar) {
        this.id_nivel_educativo = id_nivel_educativo;
        this.id_grado_escolar = id_grado_escolar;
    }

    protected trdd_c_nied_gres(Parcel in) {
        id_nivel_educativo = in.readInt();
        id_grado_escolar = in.readInt();
    }

    public static final Creator<trdd_c_nied_gres> CREATOR = new Creator<trdd_c_nied_gres>() {
        @Override
        public trdd_c_nied_gres createFromParcel(Parcel in) {
            return new trdd_c_nied_gres(in);
        }

        @Override
        public trdd_c_nied_gres[] newArray(int size) {
            return new trdd_c_nied_gres[size];
        }
    };
